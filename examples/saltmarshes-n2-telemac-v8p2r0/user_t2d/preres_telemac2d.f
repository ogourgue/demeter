...
!
! ----------------------------------------------------------------------
! START TIGER: DEMETER COUPLING VARIABLES
!
      DOUBLE PRECISION U2, TAU, MWD, TAUC, FACTOR
!
...
! START TIGER: DEMETER COUPLING VARIABLES
!
!     THE SUBROUTINE IS CALLED TWICE DURIG EACH TIME STEP, SO EACH
!     CONTRIBUTION OF TIMELY AGGREGATED VALUES (HYDROPERIOD, IMPULSE)
!     ARE MULTIPLIED BY FACTOR = 0.5.
      FACTOR = .5
!
      IF(AT - AT0 > 0.D0) THEN
        DO I = 1, NPOIN
...
!         HYDROPERIOD
          IF(H%R(I) > MWD) THEN
            TH%R(I) = TH%R(I) + DT * FACTOR
          ENDIF
!
!         BOTTOM SHEAR STRESS
          U2 = UN%R(I) * UN%R(I) + VN%R(I) * VN%R(I)
          TAU = .5 * CF%R(I) * ROEAU * U2
!
!         EXCEEDING BOTTOM SHEAR IMPULSE
          IF((TAU > TAUC).AND.(H%R(I) > MWD)) THEN
            JB%R(I) = JB%R(I) + (TAU - TAUC) * DT * FACTOR
          ENDIF
!
        ENDDO
      ENDIF
