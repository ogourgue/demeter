...
!
     ...
     &                 INCLUS(COUPLING,'KHIONE'))
!
! ----------------------------------------------------------------------
! START TIGER: DEMETER COUPLING VARIABLES
!
!     HYDROPERIOD
      CALL BIEF_ALLVEC(1, TH, 'TH    ', IELMT, 1, 2, MESH)
      CALL ADDBLO(VARSOR, TH)
      CALL OS('X=0     ', X = TH)
!
!     EXCEEDING BOTTOM SHEAR IMPULSE
      CALL BIEF_ALLVEC(1, JB, 'JB    ', IELMT, 1, 2, MESH)
      CALL ADDBLO(VARSOR, JB)
      CALL OS('X=0     ', X = JB)
!
!     COVERAGE
      CALL BIEF_ALLVEC(1, COV, 'COV   ', IELMT, 1, 2, MESH)
      CALL ADDBLO(VARSOR, COV)
      CALL OS('X=0     ', X = COV)
!
!     AGE
      CALL BIEF_ALLVEC(1, AGE, 'AGE   ', IELMT, 1, 2, MESH)
      CALL ADDBLO(VARSOR, AGE)
      CALL OS('X=0     ', X = AGE)
!
! END TIGER
! ----------------------------------------------------------------------
